...
test('所有 human 命令分支、空列表和 null 值都有稳定展示', () => {
  assert.match(
    renderHuman('config.init', { created: true, config_path: '/config.json' }, { io: humanIo }),
    /已创建/,
  );
  assert.match(
    renderHuman(
...
test('外部 adapter 防御缺少配置、未知凭据类型与 DevUC 取消', async () => {
  await assert.rejects(createDevucClient({ timeoutMs: 100 }).login('a', 'b'), {
    code: 'CONFIG_ERROR',
  });
  const controller = new AbortController();
  ...
  controller.abort();
  await assert.rejects(cancelledRequest, { code: 'CANCELLED' });
  assert.throws(
    () =>
...
test('CLI 预取消、非法认证选择与 --output=human 推断分支稳定', async () => {
  ...
  assert.equal(
    await runCli(['auth', 'status', '--output=human'], {
      io: human.io,
      configStore: configStore(),
      credentialStore: new MemoryCredentialStore(),
    }),
