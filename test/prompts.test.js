...
function assertGreenSuccessIcon(value) {
  assert.equal(value, '\u001B[32m✓\u001B[39m');
  assert.equal(stripVTControlCharacters(value), '✓');
}
...
test('登录提示使用选择、掩码输入、自定义流和 AbortSignal', async () => {
  ...
  assertGreenSuccessIcon(calls[1].options.theme.prefix.done);
...
test('DevUC 账号明文保留、密码掩码保留且提示层执行校验', async () => {
  ...
  assertGreenSuccessIcon(calls[0].options.theme.prefix.done);
