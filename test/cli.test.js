...
import { captureIo, configStore, parseSingleJson, validConfig } from '../test-support/helpers.js';
...
test('private token 登录交互保存单一系统凭据记录', async () => {
  ...
  const exitCode = await runCli(['auth', 'login'], {
    ...
    devucClientFactory: () => {
      throw new Error('must not access DevUC');
    },
    codehubAdapterFactory: () => {
      throw new Error('must not access CodeHub');
    },
  });
  assert.equal(exitCode, 0);
  assert.deepEqual(parseSingleJson(capture.stdout), {
...
test('登录取消与凭据保存失败映射稳定错误且 stdout 为空', async () => {
  for (const [prompter, store, expectedCode, expectedExit] of [
    [
      {
        chooseAuthenticationType: async () => {
...
test('auth status 只读取当前 origin 凭据，未登录字段为 null', async () => {
  for (const credential of [null, privateCredential('secret')]) {
    const { io, capture } = captureIo();
    let networkCalls = 0;
    const store = new MemoryCredentialStore(credential ? [[ORIGIN, credential]] : []);
...
test('所有业务命令输出规定的直接 JSON 结构', async () => {
  const cases = [
    ...
    [
      ['mr', 'commits', '17', '--project-id', '9001'],
      'mergeRequests.commits',
      [
        {
...
test('评论正文和 severity 原样传递且空正文在网络前拒绝', async () => {
  ...
  let adapterCalls = 0;
  const exit = await runCli(['mr', 'comment', 'create', '17', '--project-id', '9001', '--body='], {
    io: invalid.io,
    configStore: configStore(),
    credentialStore: new MemoryCredentialStore([[ORIGIN, privateCredential('secret')]]),
    codehubAdapterFactory: () => {
...
test('参数、配置、HTTP、网络和写入未知错误保持 stdout/stderr 与退出码契约', async () => {
  const cases = [
    ...
    [
      ...
      { apiError: new CliError('WRITE_RESULT_UNKNOWN') },
      8,
...
function failCredentialStore() {
  return {
    ...
    clear: async () => {
      throw new Error('must not clear credentials');
    },
  };
}
